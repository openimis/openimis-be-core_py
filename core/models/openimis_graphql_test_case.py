...
class openIMISGraphQLTestCase(GraphQLTestCase):
    ...
    def get_mutation_result(self, mutation_uuid, token):
        content= None
        while True:
            ...
            response = self.query(f'''
                {{
                mutationLogs(clientMutationId: "{mutation_uuid}")
                {{
    ...
    def send_mutation(self, mutation_type, input_params,token, follow = True):
        ...
        if follow:
            return self.get_mutation_result(input_params['clientMutationId'],token)
        else:
            return json.loads(response.content)
